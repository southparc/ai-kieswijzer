...
CREATE TABLE IF NOT EXISTS public.party_votes (
  ...
  vote_position integer NOT NULL CHECK (vote_position IN (-1, 0, 1)), -- -1 = against, 0 = abstain, 1 = for
  note text,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.party_program_positions (
  ...
  party_name text NOT NULL,
  statement_id text NOT NULL,
  program_position integer NOT NULL CHECK (program_position IN (-1, 0, 1)), -- -1 = disagree, 0 = neutral, 1 = agree
...
AS $$
  SELECT 
    ...
    ppp.program_position,
    ppp.confidence as program_confidence,
    pv.vote_position as voting_position,
    COUNT(pv.id) as vote_count,
    MAX(vr.vote_date) as latest_vote_date
  FROM public.party_program_positions ppp
  FULL OUTER JOIN (
    SELECT vr.statement_id, pv.party_name, pv.vote_position, vr.vote_date, pv.id
  ...
  GROUP BY ppp.statement_id, vr.statement_id, ppp.program_position, ppp.confidence, pv.vote_position
