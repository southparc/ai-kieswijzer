-- Add metadata column to chunks table for enhanced content analysis
ALTER TABLE public.chunks ADD COLUMN IF NOT EXISTS metadata jsonb;

-- Create GIN index on metadata JSONB column for theme filtering
CREATE INDEX IF NOT EXISTS idx_chunks_metadata_theme ON public.chunks USING gin (metadata);

-- Create function for quality analysis
...
-- Add RPC function for enhanced RAG search with theme filtering
...
-- Create function to get party coverage statistics
